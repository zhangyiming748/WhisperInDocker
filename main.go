...
func main() {
	...
	for _, file := range files {
		...
		cmd := exec.Command("whisper", file, "--threads", "0", "--model", constant.GetModel(), "--model_dir", "/root/app", "--output_format", "srt", "--prepend_punctuations", ",.?", "--language", constant.GetLanguage(), "--output_dir", constant.GetRoot(), "--verbose", "True")
