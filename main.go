...
func main() {
	...
	for _, file := range files {
		...
		cmd := exec.Command("whisper", file, "--threads", "0", "--model", "base", "--model_dir", constant.GetModel(), "--output_format", "srt", "--prepend_punctuations", ",.?", "--language", constant.GetLanguage(), "--output_dir", constant.GetRoot(), "--verbose", "True")
